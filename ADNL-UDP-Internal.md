...
1. 我們取她的 ED25519 密鑰 `fZnkoIAxrTd4xeBgVpZFRm5SvVvSx7eN3Vbe8c83YMk`，從 base64 解碼
2. 取她的IP地址 `1091897261` 並將其轉換為可讀格式，使用 [服務](https://www.browserling.com/tools/dec-to-ip)，得到 `65.21.7.173`
3. 結合端口號，得到 `65.21.7.173:15813` 並建立 UDP 連接。

我們想要打開一個通道與節點進行持續訊息交流，並且作為主要任務 - 從它那裡獲取已簽名地址列表。

為此，我們將生成兩條消息，第一條是[創建通道](https://github.com/ton-blockchain/ton/blob/master/tl/generate/scheme/ton_api.tl#L129)：
```
adnl.message.createChannel key:int256 date:int = adnl.Message
```
這裡有兩個參數 - 密鑰和日期。對於日期，我們將指定當前 unix 時間戳。

而對於密鑰- 我們需要專門為該通道生成新的 ED25519 私鑰/公鑰對，它們將用於初始化[共享加密密鑰](/ADNL-TCP-Liteserver.md#%D0%BF%D0%BE%D0%BB%D1%83%D1%87%D0%B5%D0%BD%D0%B8%D0%B5-%D0%BE%D0%B1%D1%89%D0%B5%D0%B3%D0%BE-%D0%BA%D0%BB%D1%8E%D1%87%D0%B0-%D0%BF%D0%BE-ecdh)。

在消息的 `key` 參數中，我們將指定我們生成的公鑰，而私鑰則暫時記住。
...
```
作為 `query_id`，我們生成隨機的 32 個字節，作為 `query`，我們使用我們的主要查詢，[包裝為字節數組](/TL.md#encoding-bytes-to-tl)。 
...
adnl.packetContents
   ...
   signature:flags.11?bytes -- 簽名
   rand2:bytes -- 隨機 7 或 15 個字節
         = adnl.PacketContents
...
初始交換信息時，在通道外，封包的序列化內容結構以服務器公鑰為前綴—— 32 字節，我們的公鑰為 32 字節，內容結構序列化 TL 的 sha256 哈希封包的 - 32 字節。 

封包內容使用[共享密鑰](/ADNL-TCP-Liteserver.md#%D0%BF%D0%BE%D0%BB%D1%83%D1%87%D0%B5%D0%BD%D0%B8%D0%B5-%D0%BE%D0%B1%D1%89%D0%B5%D0%B3%D0%BE-%D0%BA%D0%BB%D1%8E%D1%87%D0%B0-%D0%BF%D0%BE-ecdh) 從我們的私鑰和服務器的公鑰中得到。

序列化我們的包內容結構，逐字節解析：
```
89cd42d1 -- TL ID adnl.packetContents
0f 4e0e7dd6d0c5646c204573bc47e567 -- rand1, 15 (0f) 隨機字節
d9050000 -- 標誌 (0x05d9) -> 0b0000010111011001
                                                                        -- 來自（存在是因為標誌位 0 = 1）
c6b41348 -- TL ID pub.ed25519
    afc46336dd352049b366c7fd3fc1b143a518f0d02d9faef896cb0155488915d6——密鑰：int256
                                                                        -- 消息（存在是因為標誌的第 3 位 = 1）
02000000 -- 向量 adnl.Message，大小為 2 的消息
    bbc373e6 -- TL ID adnl.message.createChannel
    ...
    7af98bb4 -- TL ID [adnl.message.query](/)
    d7be82afbc80516ebca39784b8e2209886a69601251571444514b7f17fcd8875 -- query_id
    04 ed4879a9 000000 -- 查詢（字節大小 4，填充 3）
...
0f 2b6a8c0509f85da9f3c7e11c86ba22 -- rand2, 15 (0f) 隨機字節
```

序列化後 - 我們需要使用我們之前生成並記住的私有 ED25519 客戶端密鑰（而不是通道）對生成的字節數組進行簽名。 當我們收到簽名（64 字節大小）後，我們需要將其添加到包中，再次序列化，但將第 11 位添加到標誌中，這意味著簽名的存在：
...
555c8763 -- reinit_date（存在是因為標誌位 10 = 1，通常是初始化日期）
00000000 -- dst_reinit_date（存在是因為標誌位 10 = 1）
40 b453fbcbd8e884586b464290fe07475ee0da9df0b8d191e41e44f8f42a63a710 -- 簽名（存在是因為標誌位 11 = 1），（字節大小 64，填充 3）
    341eefe8ffdc56de73db50a25989816dda17a4ac6c2f72f49804a97ff41df502 --
    000000 --
0f 2b6a8c0509f85da9f3c7e11c86ba22 -- rand2, 15 (0f) 隨機字節
```
現在我們有一個組裝、簽名和序列化的封包，它是一個字節數組。

為了讓收件人隨後驗證其完整性，我們需要計算其 sha256 哈希值。 

例如，讓它成為“408a2a4ed623b25a2e2ba8bbe92d01a3b5dbd22c97525092ac3203ce4044dcd2”。

現在讓我們使用[共享密鑰](/ADNL-TCP-Liteserver.md#%D0%BF%D0%BE%D0%BB%D1%83%D1%87%D0%B5%D0%BD%D0%B8%D0%B5-%D0%BE%D0%B1%D1%89%D0%B5%D0%B3%D0%BE-%D0%BA%D0%BB%D1%8E%D1%87%D0%B0-%D0%BF%D0%BE-ecdh)從我們的私鑰和服務器的公鑰（不是通道密鑰）中獲得。

我們幾乎準備好發送了，它仍然是[計算 ID](/ADNL-TCP-Liteserver.md#get-id-key) 服務器密鑰的 ED25519 並將所有內容連接在一起：
```
daa76538d99c79ea097a67086ec05acca12d1fefdbc9c96a76ab5a12e66c7ebb -- 服務器密鑰 ID
...
作為響應，我們將收到一個結構相似但消息不同的封包。 它將包括：
```

68426d4906bafbd5fe25baf9e0608cf24fffa7eca0aece70765d64f61f82f005 -- 我們密鑰的 ID
2d11e4a08031ad3778c5e060569645466e52bd1bd2c7b78ddd56def1cf3760c9 -- 服務器公鑰，用於共享密鑰
...
```
89cd42d1 -- TL ID adnl.packetContents
0f 985558683d58c9847b4013ec93ea28 -- rand1, 15 (0f) 隨機字節
...
02000000 -- 消息（存在是因為第 3 個標誌位 = 1）
    691ddd60 -- TL ID adnl.message.confirmChannel
    db19d5f297b2b0d76ef79be91ad3ae01d8d9f80fab8981d8ed0c9d67b92be4e3 -- 密鑰（服務器通道公鑰）
    d59d8e3991be20b54dde8b78b3af18b379a62fa30e64af361c75452f6af019d7 -- peer_key（我們的公共頻道密鑰）
    94848863——日期
...
40 5c26a2a05e584e9d20d11fb17538692137d1f7c0a1a3c97e609ee853ea9360ab6 -- 簽名（存在是因為標誌位 11 = 1），（字節大小 64，填充 3）
    d84263630fe02dfd41efb5cd965ce6496ac57f0e51281ab0fdce06e809c7901 --
    000000 --
0f c3354d35749ffd088411599101deb2 -- rand2, 15 (0f) 隨機字節
```

服務器用兩條消息回答了我們：`adnl.message.confirmChannel` 和 `adnl.message.answer`。 

有了 adnl.message.answer 就簡單了，就是對我們請求的 dht.getSignedAddressList 的回答，我們會在 DHT 的文章中分析。

讓我們關注 `adnl.message.confirmChannel`，這意味著服務器已經確認頻道的創建並向我們發送了它的公共頻道密鑰。 現在，有了我們的私人頻道密鑰和服務器的公共頻道密鑰，我們可以計算[共享密鑰](/ADNL-TCP-Liteserver.md#%D0%BF%D0%BE%D0%BB%D1%83%D1%87%D0%B5%D0%BD%D0%B8%D0%B5-%D0%BE%D0%B1%D1%89%D0%B5%D0%B3%D0%BE-%D0%BA%D0%BB%D1%8E%D1%87%D0%B0-%D0%BF%D0%BE-ecdh)。

現在我們已經計算出共享通道密鑰，我們需要從中生成 2 個密鑰 - 一個用於加密傳出消息，另一個用於解密傳入消息。 
...
第二把鑰匙：3322BBAA
```
仍然需要確定將哪個密鑰用於什麼，我們可以通過將我們的公共頻道密鑰的 ID 與服務器頻道的公共密鑰的 ID 進行比較，將它們轉換為數字形式 - uint256 來完成此操作。 

此方法用於確保服務器和客戶端確定將哪個密鑰用於什麼。 
如果服務器使用第一個密鑰進行加密，那麼使用這種方法，客戶端將始終使用它進行解密。
...
所有後續數據封包交換都將在通道內進行，並且將使用新密鑰進行加密。 
讓我們在新創建的通道內發送相同的 `dht.getSignedAddressList` 請求以查看差異。

讓我們使用相同的 adnl.packetContents 結構收集頻道的包內容：
```
89cd42d1 -- TL ID adnl.packetContents
0f c1fbe8c4ab8f8e733de83abac17915 -- rand1, 15 (0f) 隨機字節
c4000000 -- 標誌 (0x00c4) -> 0b0000000011000100
                                                                        -- 消息（因為第 2 位 = 1）
7af98bb4 -- TL ID adnl.message.query
fe3c0f39a89917b7f393533d1d06b605b673ffae8bbfab210150fe9d29083c35 -- query_id
04 ed4879a9 000000 -- 查詢（我們的 dht.getSignedAddressList 用填充 3 字節打包）
0200000000000000 -- seqno（因為標誌的第 6 位 = 1），2 tk 是我們的第二條消息
0100000000000000 -- confirm_seqno (7th flag bit = 1), 1 tk 是從服務器收到的最後一個seqno
07 e4092842a8ae18 -- rand2, 7 (07) 隨機字節
```

通道中的數據封包非常簡單，主要由序列 (seqno) 和消息本身組成。

序列化後，和上次一樣，我們從內容中計算出 sha256 哈希值。 

然後我們使用用於通道傳出數據包的密鑰加密數據封包的內容。 
[計算](/ADNL-TCP-Liteserver.md#additional-technical-handshake-details) `pub.aes` 我們傳出消息加密密鑰的 ID，並構建我們的封包：
```
bcd1cf47b9e657200ba21d94b822052cf553a548f51f539423c8139a83162180 -- 我們傳出消息加密密鑰的 ID
...
#### 其他消息類型
對於基本通信，我們使用了類似 `adnl.message.query` 和 `adnl.message.answer` 的消息，我們在上面討論過，但在某些情況下也可能使用其他類型的消息，我們將在本節中討論。

##### adnl.message.part
此消息類型是其他可能消息類型之一的一部分，例如 `adnl.message.answer`。 
當消息太大而無法在單個 UDP 數據報中傳輸時，使用此傳輸方法。
```
adnl.message.part
hash:int256 -- 原始消息的 sha256 哈希
total_size:int -- 原始消息大小
offset:int -- 從原始消息開始的偏移量
data:bytes -- 原始消息的數據塊
    = adnl.message;
```
因此，為了組裝原始消息，我們需要得到幾個部分，並根據偏移量將它們添加到一個單字節數組中。 
然後將其作為消息處理（根據此數組中的前綴）。

##### adnl.message 自定義
```
adnl.message.custom data:bytes = adnl.message;
```
當更高層的邏輯不符合請求-響應格式時使用此類消息，這種類型的消息允許您將處理完全移動到更高層，因為消息只攜帶一個字節數組，沒有 query_id 和其他領域。 

例如，在 RLDP 中使用這種類型的消息，因為對許多請求只能有一個響應，並且此邏輯由 RLDP 本身控制。
